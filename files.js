    ...
    let state = {
        ...
        timeline: timeline.map((e) => {
            ...
            if (t.glyph) {
                let iClone = new Map();
                let oClone = new Map();
                for (let [k, v] of e.inputs.entries()) {
                    iClone.set(k, v.toString());
                }
                for (let [k, v] of e.outputs.entries()) {
                    oClone.set(k, v.toString());
                }
                t.inputs = Object.fromEntries(iClone);
                t.outputs = Object.fromEntries(oClone);
...
function loadState(data) {
    ...
    state.timeline.forEach((e) => {
        if (e.glyph) {
            e.inputs = new Map(Object.entries(e.inputs).map((k) => {
                k[1] = BigInt(k[1]);
                return k
            }));
            e.outputs = new Map(Object.entries(e.outputs).map((k) => {
                k[1] = BigInt(k[1]);
                return k
            }));
